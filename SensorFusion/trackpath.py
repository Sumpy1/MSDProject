"""Algorithm for gait tracking using IMU data developed by XIO technologies
Very similar to what we are trying to have eventually where we can track any movement & path of IMU unit or sensor
in 3d space.

Tracks the position of an IMU attached to a subject's foot during walking using Python. The algorithm uses Fusion to obtain a measurement
...
obtain a measurement of velocity. The measurement of velocity is corrected for drift using a zero-velocity detection algorithm and then integrated to obtain the measurement of position.
...
data = numpy.genfromtxt("SenseData.csv", delimiter=",", skip_header=1)
